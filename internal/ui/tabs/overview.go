...
func RenderOverview(s *data.AppState, container, titleStyle, labelStyle, valueStyle lipgloss.Style, su, w, a, t, mu, bg, p, b compat.AdaptiveColor, availHeight int) string {
	...
	memBlock := lipgloss.JoinVertical(lipgloss.Left,
		fwLine(memVal, idx),
		fwLine(memBar, idx),
		fwLine(labelStyle.Render(util.FormatBytes(memUsed))+sp(" / ")+labelStyle.Render(util.FormatBytes(memTotal)), idx),
	)
	...
	diskBlock := lipgloss.JoinVertical(lipgloss.Left,
		fwLine(diskVal, idx),
		fwLine(diskBar, idx),
		fwLine(labelStyle.Render(util.FormatBytes(diskUsed))+sp(" / ")+labelStyle.Render(util.FormatBytes(diskTotal)), idx),
	)
	...
	uptimeStr := "Loading..."
	if s.HostInfo != nil {
		uptimeStr = util.FormatDuration(time.Duration(s.HostInfo.Uptime) * time.Second)
	}
	...
	quickStatsBlock := lipgloss.JoinVertical(lipgloss.Left,
		...
		fwLine(lipgloss.JoinHorizontal(lipgloss.Top,
			...
			valueStyle.Render(fmt.Sprintf("%d", s.ProcessCount)),
		), idx),
		fwLine(lipgloss.JoinHorizontal(lipgloss.Top,
		...
		fwLine(lipgloss.JoinHorizontal(lipgloss.Top,
			labelStyle.Render("Health:"+sp("      ")),
			valueStyle.Render(fmt.Sprintf("%d%%", s.HealthScore)),
		), idx),
	)
	...
	if s.Swap > 0 && s.SwapInfo != nil {
		...
		swapBlock = lipgloss.JoinVertical(lipgloss.Left,
			fwLine(swapVal, idx),
			fwLine(swapBar, idx),
			fwLine(labelStyle.Render(util.FormatBytes(s.SwapInfo.Used))+sp(" / ")+labelStyle.Render(util.FormatBytes(s.SwapInfo.Total)), idx),
		)
	}
	...
	if len(s.Processes) > 0 {
		topStr = ""
		limit := 4
		for i := 0; i < len(s.Processes) && i < limit; i++ {
			p := s.Processes[i]
			name := p.Name
			if len(name) > 12 {
				name = name[:9] + "..."
