...
func RenderSystem(s *data.AppState, container, titleStyle, labelStyle, valueStyle lipgloss.Style, t, mu, p, b, bg compat.AdaptiveColor, availHeight int) string {
	...
	uptimeLoad := lipgloss.JoinVertical(lipgloss.Left,
		fwLine(lipgloss.JoinHorizontal(lipgloss.Top, labelStyle.Render("Uptime:"+sp("    ")), valueStyle.Render(util.FormatDuration(time.Duration(s.HostInfo.Uptime)*time.Second))), idx),
	...
	uptime := lipgloss.JoinVertical(lipgloss.Left,
		...
		fwLine(lipgloss.JoinHorizontal(lipgloss.Top, labelStyle.Render("System:"+sp(" ")), valueStyle.Render(util.FormatDuration(time.Since(bootTime)))), idx),
		fwLine(lipgloss.JoinHorizontal(lipgloss.Top, labelStyle.Render("Monitor:"), valueStyle.Render(util.FormatDuration(time.Since(s.StartTime)))), idx),
	)
	...
	memInfo := lipgloss.JoinVertical(lipgloss.Left,
		...
		fwLine(lipgloss.JoinHorizontal(lipgloss.Top, labelStyle.Render("Used:"+sp("      ")), valueStyle.Render(fmt.Sprintf("%s"+sp(" ")+"(%.2f%%)", util.FormatBytes(memUsed), memUsedPct))), idx),
	)
